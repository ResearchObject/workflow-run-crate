...
from pycompss.api.parameter import DICTIONARY_IN_DELETE
from pycompss.api.parameter import IN_DELETE
...
# @task(returns=1, file_out_fig=FILE_OUT, coord_sta={Type: DICTIONARY_IN, Depth: 1}, GRD_sta={Type: DICTIONARY_IN, Depth: 2})
def _run_btbb(config, st, st_CF, t_begin,
             coord_eq, coord_sta, rec_memory, grid1,
             GRD_sta, file_out_fig):
    #---init map projection--------------------------------------------------
    ...
    #------------------------------------------------------------------------
    return _run_btbb(config, st, st_CF, t_begin,
                     coord_sta, GRD_sta, coord_eq,
                     file_out_fig, rec_memory)

def run_btbb_no_fig(args):
    return _run_btbb_no_fig(*args)

# @task(returns=1, coord_sta={Type: DICTIONARY_IN, Depth: 1}, GRD_sta={Type: DICTIONARY_IN, Depth: 2})
def _run_btbb_no_fig(config, st, st_CF, t_begin,
                    coord_eq, coord_sta, rec_memory, grid1,
                    GRD_sta, file_out_fig):
    #---init map projection--------------------------------------------------
    if  grid1.proj_name!= 'NONE':
...
def _run_btbb(config, st, st_CF, t_begin,
    ...
    if config.ncpu > 1 and config.recursive_memory:
        # Parallelization is done here
        # Note that only if recursive memory is set, since it can not be
        # done if it is already in a multiprocessing pool.
    ...
    for out in outputs:
        ...
        # ##########################
        # import sys
        # sys.stdout.write("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX 1 \n")
        # for k in GRD_sta.keys():
        #     for v in GRD_sta[k].keys():
        #         sys.stdout.write("k: " + str(k) + " v: " + str(v) + "\n")
        #         sys.stdout.write(str(GRD_sta[k][v]) + "\n")
        # sys.stdout.write("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX 1 \n")
        # sys.stdout.write(str(GRD_sta[sta2]) + "\n")
        # sys.stdout.write("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX 2 \n")
        # sys.stdout.write(str(GRD_sta[sta2][wave2]) + "\n")
        # sys.stdout.write("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX 3 \n")
        # sys.stdout.write(str(GRD_sta[sta1]) + "\n")
        # sys.stdout.write("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX 4 \n")
        # sys.stdout.write(str(GRD_sta[sta1][wave1]) + "\n")
        # sys.stdout.write("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX 5 \n")
        # sys.stdout.flush()
        # ##########################
        delta_tt_array =\
