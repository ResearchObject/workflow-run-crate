...
def bp_plot(
    ...
    ax1_xy.set_title(
        "Date: %s, Time: %s.%03d - %s.%03d (%s - %s s)"
        % (
            tt1.date,
            tt1.strftime("%H:%M:%S"),
            int(round(tt1.microsecond / 1000.0)),
            tt2.strftime("%H:%M:%S"),
            int(round(tt2.microsecond / 1000.0)),
    ...
    if coord_eq:
        ax1_xy.scatter(
            coord_eq[0], coord_eq[1], marker="*", s=eq_smbl_size, linewidths=1, c="w"
        )
    for sta in coord_sta:
        x_sta, y_sta = coord_sta[sta]
        ax1_xy.scatter(
            x_sta, y_sta, marker="^", s=sta_smbl_size, linewidths=1, c="w", alpha=0.79
        )
    ...
    if trigger is not None:
        ax1_xy.scatter(
            xx_max, yy_max, marker="*", s=trig_smbl_size, linewidths=1, c="g"
        )
        ax1_yz.scatter(
            zz_max, yy_max, marker="*", s=trig_smbl_size, linewidths=1, c="g"
        )
        ax1_xz.scatter(
            xx_max, zz_max, marker="*", s=trig_smbl_size, linewidths=1, c="g"
        )
    ...
    if trigger is not None:
        ...
        axes, cb11 = proj_grid.plot(
            max_ijk, handle=True, ax_xy=ax2_xy, vmin=lcc_min, vmax=lcc_max, cmap=scmap2
        )
        ...
        if trigger.lon:
            ax2_xy.set_title(
                "%sLon: %.4f, Lat: %.4f, Depth: %.3f km"
                % (t_str, trigger.lon, trigger.lat, trigger.z)
            )
        else:
            ax2_xy.set_title(
                "%sX: %.2f km, Y: %.2f km, Depth: %.2f km"
    ...
    if config.plot_time_win_size is not None:
        ...
        st_plt.trim(
            config.starttime + t_begin - t_extra, config.starttime + t_end + t_extra
        )
        st_CF_plt.trim(
            config.starttime + t_begin - t_extra, config.starttime + t_end + t_extra
        )
    ...
    for sta in set(tr.stats.station for tr in st_plt):
        ...
        for wave in config.wave_type:
            ...
            if ("%s" % sta, "%s" % wave) in unused_CF:
                ...
                linestyle = "-"
            ax3.plot(time_CF, ydata, color=color, linestyle=linestyle, rasterized=True)
    ...
    if trigger is not None and not trigger.valid:
        fig.suptitle(
            "INVALID TRIGGER: arrival time difference larger than "
            "{} s (dt_min)".format(config.dt_min),
            fontsize=24,
        )
...
@task(file_out_fig=FILE_OUT, coord_sta={Type: DICTIONARY_IN, Depth: 2})
def plt_SummaryOut(
    config,
    grid1,
    st_CF,
    ...
    ax1_xy.scatter(
        x_trig, y_trig, marker="*", s=trig_smbl_size, linewidths=0.5, c="g", alpha=0.7
    )
    ax1_yz.scatter(
        z_trig, y_trig, marker="*", s=trig_smbl_size, linewidths=0.5, c="g", alpha=0.7
    )
    ax1_xz.scatter(
        x_trig, z_trig, marker="*", s=trig_smbl_size, linewidths=0.5, c="g", alpha=0.7
    )
    for sta in coord_sta:
        x_sta, y_sta = coord_sta[sta]
        ax1_xy.scatter(
            x_sta, y_sta, marker="^", s=sta_smbl_size, linewidths=1, c="k", alpha=0.79
        )
        trans = ax1_xy.transData + ax1_xy.transAxes.inverted()
        x_sta_ax, y_sta_ax = trans.transform((x_sta, y_sta))
        ax1_xy.text(
    ...
    if coord_eq:
        ax1_xy.scatter(
            coord_eq[0], coord_eq[1], marker="*", s=eq_smbl_size, linewidths=1, c="r"
        )
        ax1_yz.scatter(
            coord_eq[2], coord_eq[1], marker="*", s=eq_smbl_size, linewidths=1, c="r"
        )
        ax1_xz.scatter(
            coord_eq[0], coord_eq[2], marker="*", s=eq_smbl_size, linewidths=1, c="r"
        )
    if coord_jma:
        ax1_xy.scatter(
            coord_jma[0], coord_jma[1], marker="o", s=eq_smbl_size, linewidths=1, c="m"
        )
        ax1_yz.scatter(
            coord_jma[2], coord_jma[1], marker="o", s=eq_smbl_size, linewidths=1, c="m"
        )
        ax1_xz.scatter(
            coord_jma[0], coord_jma[2], marker="o", s=eq_smbl_size, linewidths=1, c="m"
        )
    ...
    ax3 = fig.add_subplot(122)
    # Data and CF time def. moved to the station plotting loop
    # time here is only for estimating limits
    max_nptssta = max([tr.stats.npts for tr in st])
    time = np.arange(max_nptssta) / st[0].stats.sampling_rate
    # time = np.arange(st[0].stats.npts) / st[0].stats.sampling_rate
    time += config.cut_start
    ax3.set_xlim(min(time), max(time))
    # --- config.starttime  plots should be relative to this
    sta_y = [coord_sta[sta][1] for sta in coord_sta]
    ax3.set_ylim(min(sta_y), max(sta_y))
    ax3.set_xlabel("Time[sec]")
    ax3.set_ylabel("Y[km]")
    ...
    for sta in set(tr.stats.station for tr in st_plt):
        ...
        if plot_waveforms:
            ...
            time = np.arange(tr.stats.npts) / tr.stats.sampling_rate
            time += config.cut_start
            data_shift = tr.stats.starttime - config.starttime
            time += data_shift
            # Project signal to Axes coordinates:
            signal = tr.data / abs(tr.max()) * 0.05 + y_sta_ax
            xydata = np.dstack((np.zeros_like(signal), signal))[0]
        ...
        for wave in config.wave_type:
            ...
            #
            time_env = np.arange(tr_CF.stats.npts) / tr_CF.stats.sampling_rate
            time_env += config.cut_start
            cf_shift = tr_CF.stats.starttime - config.starttime
            time_env += cf_shift
            #
            signal = tr_CF.data / abs(tr_CF.max()) * 0.05 + y_sta_ax
            xydata = np.dstack((np.zeros_like(signal), signal))[0]
    ...
    for t in triggers:
        ax3.axvspan(t.beg_win, t.end_win, facecolor="r", alpha=0.1)

    ax3.axvline(t_bb[0] + config.cut_start, linewidth=1, color="b", alpha=0.9)
    ax3.axvline(
        t_bb[-1] + time_lag + config.cut_start, linewidth=1, color="b", alpha=0.9
    )
    ax3.autoscale(enable=True, axis="y", tight=False)
