...
from load_ga_export import load_ga_history_export, GalaxyJob, GalaxyDataset
...
class ProvenanceProfile:
    ...
    def generate_prov_doc(self) -> Tuple[str, ProvDocument]:
        ...
        self.document.add_namespace(UUID)
                    ...
                    for v in value:
                        ...
                        for d in self.datasets:
                            ...
                            if v in (
                                ...
                                if item == "outputs":
                                    self.generate_output_prov(d, process_run_id, tool_id)
                # else:
    ...
    def generate_output_prov(
        ...
        if isinstance(final_output, Dict):
            ...
            entity = self.declare_artefact(final_output)
            if name is not None:
                name = urllib.parse.quote(str(name), safe=":/,#")
                # FIXME: Probably not "main" in nested workflows
                role = self.wf_ns[f"main/{name}/{final_output['dataset_uuid']}"]
            else:
                role = self.wf_ns["main/%s" % final_output['dataset_uuid']]
